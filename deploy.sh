...
if [ "${db_engine}" = "postgres" ];
    ...
    then 
        dbAPIMUrl="jdbc:sqlserver://$dbHost:$dbPort;databaseName=WSO2AM_DB;SendStringParametersAsUnicode=false"
        dbAPIMSharedUrl="jdbc:sqlserver://$dbHost:$dbPort;databaseName=WSO2AM_SHARED_DB;SendStringParametersAsUnicode=false"
elif [ "${db_engine}" = "oracle" ];
    then 
